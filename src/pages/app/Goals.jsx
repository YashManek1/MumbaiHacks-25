...
import { FiPlus, FiMoreVertical, FiTarget, FiX, FiTrendingUp, FiEdit2, FiTrash2, FiDollarSign, FiRefreshCw } from 'react-icons/fi';
...
const CircularProgress = ({ percentage, size = 140, strokeWidth = 12, color = "text-blue-500" }) => {
  ...
  return (
    <div className="relative inline-flex items-center justify-center" style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        <circle className="text-gray-700" strokeWidth={strokeWidth} stroke="currentColor" fill="transparent" r={radius} cx={center} cy={center} />
        <circle
          className={color}
        ...
        {showFundModal && (
          ...
          >
            ...
            >
              ...
              <div className="mb-4">
                <label className="block text-sm text-gray-300 mb-2">Amount to Transfer</label>
                <input 
                  type="number" 
                  value={fundAmount} 
                ...
                {fundAmount && parseFloat(fundAmount) > availableSavings && (
                  <p className="text-red-400 text-xs mt-1">Amount exceeds available savings</p>
                )}
              </div>
              <div className="flex gap-2 mb-4">
                ...
                <button 
                  onClick={() => setFundAmount(Math.min(500, availableSavings, remainingVal).toString())}
                  className="px-3 py-1 bg-gray-700 hover:bg-gray-600 text-gray-300 text-sm rounded-lg"
                  disabled={funding}
                >
                  $500
                ...
                <button 
                  onClick={() => setFundAmount(Math.min(remainingVal, availableSavings).toString())}
                  className="px-3 py-1 bg-gray-700 hover:bg-gray-600 text-gray-300 text-sm rounded-lg"
                  disabled={funding}
                >
                  Complete Goal
...
const GoalModal = ({ isOpen, onClose, onSave, editingGoal = null, isLoading }) => {
  ...
  useEffect(() => {
    if (editingGoal) {
      ...
      setFormData({ title: '', total: '', monthly_contribution: '', priority: 'medium', icon: '🎯' });
    }
  }, [editingGoal, isOpen]);
  ...
  return (
    ...
    >
      <motion.div 
        ...
        exit={{ scale: 0.9, opacity: 0 }}
      >
        <button onClick={onClose} className="absolute top-4 right-4 text-gray-500 hover:text-white" disabled={saving}>
          <FiX size={20} />
        </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input 
              ...
              disabled={saving}
            />
            <p className="text-xs text-gray-500 mt-1">How much you plan to save monthly for this goal</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-gray-700/50 p-4 rounded-lg">
            <p className="text-sm text-gray-400">
              💡 <span className="text-gray-300">Tip:</span> Extra money added here goes directly to your available savings and can be allocated to any of your goals.
            </p>
          </div>
...
const Goals = () => {
  ...
  const handleFundGoal = async (goalId, amount) => {
    try {
      ...
      showToast(error.response?.data?.detail || 'Failed to fund goal. Please try again.', 'error');
      throw error;
    }
  };
  ...
  const filteredGoals = filterPriority === 'all' 
    ? goals 
  ...
  return (
    <div className="bg-gray-950 text-gray-200 min-h-screen">
      ...
      >
        {/* Header */}
        <motion.div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-8" variants={itemVariants}>
          <div>
            <h1 className="text-3xl font-bold text-white">Savings & Goals</h1>
            <p className="text-gray-400 mt-1">Manage your savings and track progress towards your financial goals</p>
          </div>
        ...
        >
          <div className="bg-gray-800/50 border border-gray-700/50 p-5 rounded-xl">
            ...
            <p className="text-2xl font-bold text-white">${financialData.totalFunds.toLocaleString()}</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            >
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between text-gray-400">
                  ...
                  <span className="text-gray-200">${financialData.monthlySavingsRate.toLocaleString()}/mo</span>
                </div>
                <div className="flex justify-between text-gray-400">
                  <span>Allocated to Goals</span>
                  <span className="text-gray-200">${financialData.totalFunded.toLocaleString()}</span>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div 
              ...
              variants={itemVariants}
            >
              <FaLightbulb size={24} className="text-blue-400 flex-shrink-0 mt-0.5" />
              <div>
